...
from tensorflow.keras.layers import Input, Reshape, Dense, Dropout, UpSampling2D, Conv2D, Flatten , LeakyReLU
...
def train(BATCH_SIZE, X_train):
    ...
    for epoch in tqdm(range(200)):
        for index in range(int(X_train.shape[0]/BATCH_SIZE)):
            ...
            d.trainable = True
        g.save_weights('./Supplement/generator', True)
        d.save_weights('./Supplement/discriminator', True)
    return d, g
...
def generate(BATCH_SIZE):
    g = generator_model()
    g.load_weights('./Supplement/generator')
    noise = np.random.uniform(0, 1, (BATCH_SIZE, 100))
...
def feature_extractor():
    d = discriminator_model()
    d.load_weights('./Supplement/discriminator') 
